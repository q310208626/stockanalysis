...
public class StockBoardCurDayJobController {
    ...
    public String triggerJob() {
        try {
            JobKey jobKey = JobKey.jobKey(JobConstants.JOB_NAME_CURRENT_DAY_BOARD_RESULT_JOB);
            synchronized (this) {
    ...
    public CurDayJobResult getCurDayJobResult() {
        try {
            Optional<JobDetail> jobDetail = Optional.of(scheduler.getJobDetail(JobKey.jobKey(JobConstants.JOB_NAME_CURRENT_DAY_BOARD_RESULT_JOB)));
